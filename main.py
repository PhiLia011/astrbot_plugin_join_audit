...
PLUGIN_NAME = "astrbot_plugin_join_audit"
...
class JoinReviewPlugin(Star):
    ...
    def _is_blacklisted(self, group_id: str, user_id: str) -> bool:
        ...
        if user_id in self.blacklist.get("*", []):
            return True
        # 修复：原先 global 模式会遍历所有群记录，导致某群拉黑的人在其它群也被拒
